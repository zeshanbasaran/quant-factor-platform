...
# ---------------------------------------------------------------------
# Back-compat helper for tests: minimal factor tearsheet
# ---------------------------------------------------------------------

def build_factor_tearsheet(
    *,
    scores: pd.DataFrame,            # per-date cross-sectional scores (rows=dates, cols=symbols)
    fwd_returns: pd.DataFrame,       # per-date forward returns aligned to scores
    portfolio_returns: pd.Series,    # realized portfolio return series
) -> dict:
    """
    Minimal, dependency-light tearsheet used by tests.

    Returns:
      {
        "ic": pd.Series (per-date Spearman IC),
        "summary": pd.DataFrame with columns ["mean_return","vol","sharpe"]
      }
    """
    # --- IC series (Spearman rank corr by date) ---
    dates = scores.index.intersection(fwd_returns.index)
    ics = []
    for dt in dates:
        a = scores.loc[dt]
        b = fwd_returns.loc[dt]
        ...
        ra = s.iloc[:, 0].rank()
        rb = s.iloc[:, 1].rank()
        ics.append(float(ra.corr(rb)))
    ic = pd.Series(ics, index=dates, name="IC")

    # --- summary from portfolio returns ---
    pr = pd.Series(portfolio_returns).dropna()
    mean_ret = float(pr.mean()) if not pr.empty else np.nan
    vol = float(pr.std(ddof=0)) if len(pr) > 1 else np.nan
    sharpe = mean_ret / vol if (vol and np.isfinite(vol)) else np.nan

    summary = pd.DataFrame(
        {"mean_return": [mean_ret], "vol": [vol], "sharpe": [sharpe]},
        index=["portfolio"],
    )
