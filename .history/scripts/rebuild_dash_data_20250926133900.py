...
def load_prices_tidy(path: Path) -> pd.DataFrame:
    ...
    if "date" not in [str(getattr(df.index, "name", "")).lower()] \
       and "date" not in [str(c).lower() for c in df.columns]:
        df = df.reset_index()

    # Flatten then DEDUP column names
    df.columns = [_flatten_col(c) for c in df.columns]
    df = df.loc[:, ~pd.Index(df.columns).duplicated(keep="first")]  # <— important

    # Aliases (same as before) …
    # ...

    # Wide → long (unique id_vars!)
    if "adj_close" not in df.columns or "ticker" not in df.columns:
        wide_cols = [c for c in df.columns if c.startswith("adj_close_")]
        if wide_cols:
            long = pd.melt(
                df,
                id_vars=["date"],             # <— single, unique date column
                value_vars=wide_cols,
                var_name="tmp",
                value_name="adj_close",
            )
            long["ticker"] = (long["tmp"]
                                .str.replace("adj_close_", "", regex=False)
                                .str.upper())
            out = long[["date", "ticker", "adj_close"]]
        else:
            raise ValueError(
                f"prices.parquet is not tidy and no adj_close_* columns were found. "
    ...
    out = (out.dropna(subset=["date", "adj_close"])
              .drop_duplicates(subset=["date", "ticker"])   # <— prevent dupes
